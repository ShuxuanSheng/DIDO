...
def get_inference(network, data_loader, device, epoch,args):
    ...
    for bid, (feat, targ,ori_r,gt_v,gt_q,rpm,kf,D,_, _) in enumerate(data_loader):
        if args.arch == "resnet":

            gt_v_body = torch.einsum("atpi,atp->ati", ori_r, gt_v)
            g = torch.einsum("tpi,tp->ti", ori_r[:, -1, :, :], torch.tensor([[0, 0, 9.8]]).repeat(ori_r.shape[0], 1)).to(device)
            ...
            theta = 0.1 * (torch.rand_like(gt_v_body) - 0.5)
            noise = torch.zeros(theta.shape[0], theta.shape[1], 4).to(theta.device)
            noise[:, :, 0] = torch.cos(torch.norm(theta, dim=2))
            noise[:, :, 1:4] = theta * torch.sin(torch.norm(theta, dim=2)).unsqueeze(2)

            features = torch.cat((feat[:,0:3,:],gt_v_body.permute(0,2,1),rpm.permute(0,2,1)),dim=1)
            y1 = (kf * torch.sum(rpm.pow(2),dim=2)).unsqueeze(2) * torch.tensor([[[0.,0.,1.]]]).repeat((rpm.shape[0],rpm.shape[1],1)).to(device)
