...
class FbSequence(CompiledSequence):
    ...
    def load(self, data_path):
        ...
        self.ts = pickle_dict["t"]
        imu = pickle_dict["u"]
        v_wheel_left = pickle_dict["v_wheel_left"]
        v_wheel_right = pickle_dict["v_wheel_right"]
        v_imu_gt = pickle_dict["v_imu_gt"]
        yaw = imu[:, 2]

        # self.ts = ts  # ts of the beginning of each window
        self.features = np.concatenate([imu, v_wheel_left.unsqueeze(1)], axis=1)
        self.targets = v_imu_gt.detach().cpu().numpy()
        self.wheel_left = v_wheel_left.detach().cpu().numpy()
        self.wheel_right = v_wheel_right.detach().cpu().numpy()
        self.yaw = torch.Tensor(yaw).detach().cpu().numpy()
    ...
    def get_wheel_left(self):
        return self.wheel_left

    def get_wheel_right(self):
        return self.wheel_right
...
class FbSequenceDataset(Dataset):
    def __init__(self, root_dir, data_list, args, data_window_config, **kwargs):
        ...
        self.step_size = 1
        # logging.info(f"self.step_size = {self.step_size}")
        ...
        self.wheel_left, self.wheel_right = [], []
        self.yaw = []
        self.no_yaw_q = []
        self.gt_q = []
        self.rpm = []
        ...
        for i in range(len(data_list)):
            ...
            logging.info(f"Loading from preprocess data: {osp.join(root_dir, data_list[i])}")
            seq = FbSequence(osp.join(root_dir, data_list[i]), args, data_window_config, **kwargs) #load一个seq的数据
            feat, targ, wheel_left, wheel_right, yaw = seq.get_feature(), seq.get_target(), seq.get_wheel_left(), seq.get_wheel_right(), seq.get_yaw()
            # 把所以data_list中的seq放到一起管理
            self.features.append(feat)
            self.targets.append(targ)
            self.wheel_left.append(wheel_left)
            self.wheel_right.append(wheel_right)
            self.yaw.append(yaw)
            self.index_map += [
                [i, j]
                for j in range(0 + self.past_data_size, self.targets[i].shape[0] - self.future_data_size, self.step_size)
                # 按照从[0 + self.past_data_size,self.targets[i].shape[0] - self.future_data_size]中按照步长self.step_size生成索引
                # [19:4:end]
            ] #self.features中存放了多个seq的数据，通过map建立seq及其对应数据起始～终止索引的映射关系。
             # 一个i对应多个j，self.step_size = 4,例如[i=0,j=4][i=0,j=8][i=0,j=12][i=1,j=16]……
            # logging.info(f"begin = {0 + self.past_data_size}, end = {self.targets[i].shape[0] - self.future_data_size}, step = {self.step_size}")

        if self.shuffle:
            random.shuffle(self.index_map)

    def __getitem__(self, item):
        # 返回一个样本所包含的数据，item不是显示传递的，是自动根据batch_size和数据的总长度生成的
        ...
        # item = 3 时，self.index_map[3] 是 [seq_id=1, frame_id=8]……
        seq_id, frame_id = self.index_map[item][0], self.index_map[item][1]
        # logging.info(f"seq_id = {seq_id}, frame_id = {frame_id}")

        # in the world frame
        # 提取features中指定范围的数据,目前是20帧,即将20帧imu和wheel拼成一个feat，feat.shape = (20,7)
        # 利用20帧为一个窗口，预测一帧targ
        ...
        wheel_left = self.wheel_left[seq_id][frame_id]
        wheel_right = self.wheel_right[seq_id][frame_id]
        yaw = self.yaw[seq_id][frame_id]
        return feat.astype(np.float32).T, targ.astype(np.float32), wheel_left.astype(np.float32), wheel_right.astype(np.float32), yaw, seq_id, frame_id
    def __len__(self):
