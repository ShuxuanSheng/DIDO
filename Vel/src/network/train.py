...
def get_inference(network, data_loader, device, epoch,args):
    ...
    for bid, (feat, targ, wheel_left, wheel_right, yaw, _, _) in enumerate(data_loader):
        if args.arch == "resnet":
            feat, targ, wheel_left, wheel_right, yaw = feat.to(device), targ.to(device), wheel_left.to(device), wheel_right.to(device),yaw.to(device)
...
def do_train(network, train_loader, device, epoch, optimizer,args):
    ...
    for bid, (feat, targ, wheel_left, wheel_right, yaw, _, _) in enumerate(train_loader):
        if args.arch == "resnet":
            # feat = [gyro, acc]
            feat, targ, wheel_left, wheel_right = feat.to(device), targ.to(device), wheel_left.to(device), wheel_right.to(device)
            optimizer.zero_grad()
            # logging.info(f"feat.shape = {feat.shape}") #feat.shape = torch.Size([512, 6, 20])
            pred, pred_cov = network(feat.to(device))
            zeros_col = torch.zeros((pred.shape[0], 2)).to(device)
            # print(f"wheel_left shape = {wheel_left.shape}, zeros_col shape = {zeros_col.shape}")
            wheel_v_combined = torch.concatenate((wheel_left.unsqueeze(1), zeros_col), dim=1)
...
def arg_conversion(args):
    ...
    net_config = {
        "inter_dim": (data_window_config["past_data_size"]
