...
def do_train(network, train_loader, device, epoch, optimizer,args):
    ...
    for bid, (feat, targ, pos, vel, yaw, _, _) in enumerate(train_loader):
        if args.arch == "resnet":
            np.savetxt("diff_targ.csv", np.diff(targ.numpy(), axis=0))
            feat, targ, pos, vel, yaw = feat.to(device), targ.to(device), pos.to(device), vel.to(device), yaw.to(device)

            optimizer.zero_grad()
            # logging.info(f"feat.shape = {feat.shape}") #feat.shape = torch.Size([512, 6, 20])
            pred, pred_cov = network(feat.to(device)) #pred: gyr_z
